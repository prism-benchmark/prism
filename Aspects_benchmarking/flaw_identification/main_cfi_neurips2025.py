...
Key differences from ICLR / ICML pipelines:
  - Human review JSON uses NeurIPS rubric:
      Summary, Strengths (may embed weaknesses inside!), Weaknesses,
      Questions, Limitations.
    Some reviewers write both Strengths AND Weaknesses inside the "Strengths"
    field using markdown sub-headers, leaving "Weaknesses" empty.
    → format_human_review_text_neurips handles this by always including both.
  - Paper files are .grobid.txt (GROBID-extracted text)
  - SEA reviews use the same **Section:** format as ICLR 2025 / ICML 2025
    → only Summary / Weaknesses / Questions are kept

Three modes
-----------
  cfi_only  – Step 1 (flaw extraction) + Step 2 (validation) + CFI metrics
              → outputs to output_cfi_neurips2025/all_papers_results.jsonl
  all       – cfi_only + CPS coverage metrics
  cps_only  – CPS from a saved CFI JSONL (no paper text needed)
              → default cache: output_cfi_neurips2025/all_papers_results.jsonl

Example commands
----------------
  python main_cfi_neurips2025.py --mode cfi_only
  python main_cfi_neurips2025.py --mode all
  python main_cfi_neurips2025.py --mode cps_only
  python main_cfi_neurips2025.py --mode cfi_only --model gemini-2.5-flash
  python main_cfi_neurips2025.py --mode cfi_only --llm-type deepreview
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run flaw-identification pipeline on NeurIPS 2025 data.",
...
def process_single_paper(
    ...
    # Load human reviews (NeurIPS rubric)
