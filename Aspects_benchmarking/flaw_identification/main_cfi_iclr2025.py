"""
Flaw-Identification Pipeline – ICLR 2025 entry point.

Data paths  : <DATA_ROOT>/ICLR2025/
...
-----------
  cfi_only  – Step 1 (flaw extraction) + Step 2 (validation) + CFI metrics
              → outputs to output_cfi_iclr2025/all_papers_results.jsonl
  all       – cfi_only + CPS coverage metrics
  cps_only  – CPS from a saved CFI JSONL (no paper text needed)
              → default cache: output_cfi_iclr2025/all_papers_results.jsonl

Example commands
----------------
  python main_cfi_iclr2025.py --mode cfi_only
  python main_cfi_iclr2025.py --mode all
  python main_cfi_iclr2025.py --mode cps_only
  python main_cfi_iclr2025.py --mode cfi_only --model gemini-2.5-flash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run flaw-identification pipeline on ICLR 2025 data.",
    ...
    parser.add_argument(
        ...
        help=(
            "LLM reviewer source:\n"
            "  sea       – SEA reviews from sea_iclr2025/ (default)\n"
            "  tree      – Tree reviews from tree_iclr2025/\n"
            "  reviewer2 – Reviewer2 reviews from reviewer2_iclr2025/\n"
            "  deepreview– DeepReview JSON from deepreview_iclr2025/\n"
            "  cyclereview– CycleReview JSON from cyclereview_iclr2025/"
        ),
    )
    parser.add_argument(
        "--workers",
...
def process_single_paper(
    ...
    # Load human reviews — include Limitations field for ICLR 2025
    ...
    # Load LLM review (SEA, Tree, or Reviewer2)
    if llm_type == "tree":
    ...
    if mode == "cps_only":
        ...
        if args.workers > 1 and todo_records:
            with ThreadPoolExecutor(max_workers=args.workers) as executor:
                futures = {executor.submit(process_one_cps, r): r for r in todo_records}
