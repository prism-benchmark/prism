"""
Flaw-Identification Pipeline – ICLR 2025 entry point (Mimo v2.5 Pro).

Data paths  : E:\\Final_LLM_Reviewer_Data\\ICLR2025\\
...
Three modes
-----------
  cfi_only  – Step 1 + Step 2 + CFI recall metrics
              → outputs to output_cfi_iclr2025_mimo/all_papers_results.jsonl
  all       – cfi_only + CPS coverage metrics
  cps_only  – CPS from a saved CFI JSONL (no paper text needed)

Example commands
----------------
  python main_cfi_iclr2025_mimo.py --mode cfi_only
  python main_cfi_iclr2025_mimo.py --mode all
  python main_cfi_iclr2025_mimo.py --mode cps_only
  python main_cfi_iclr2025_mimo.py --mode cfi_only --llm-type deepreview
...
PAPER_IDS_FILE     = os.path.join(ICLR2025_DATA, "paper_ids_50_iclr2025.txt")
OUTPUT_DIR         = os.path.join(_HERE, "output_cfi_iclr2025_mimo")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run flaw-identification pipeline on ICLR 2025 data (Mimo).",
