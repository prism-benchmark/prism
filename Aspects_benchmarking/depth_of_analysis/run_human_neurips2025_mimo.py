"""
run_human_neurips2025_mimo.py  Xu ly Human reviews NeurIPS2025 bang Mimo v2.5 Pro.

Mac inh chay tren subset 50 paper IDs (paper_ids_50_neurips2025.txt).
...
def run_pipeline(paper_ids_file: str = None, run_all: bool = False):
    ...
    for paper_id in tqdm(valid_ids, desc=" Human NeurIPS2025 [Mimo]", unit="paper"):
        if is_processed(paper_id):
            continue

        print(f"\n [{paper_id}]")

        with open(os.path.join(NEURIPS2025_HUMAN_DIR, f"{paper_id}.json"), "r", encoding="utf-8", errors="replace") as f:
    ...
    print(f"\n Human NeurIPS2025 [Mimo] hoan tat! Ket qua tai: {OUTPUT_DIR}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DoA  Human NeurIPS2025 with Mimo v2.5 Pro evaluator"
    )
    parser.add_argument("--paper_ids", type=str, default=None,
                        help=f"File paper IDs (mac inh: {NEURIPS2025_PAPER_IDS_50})")
