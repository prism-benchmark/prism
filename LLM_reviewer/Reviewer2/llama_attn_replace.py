...
try:
    ...
    warnings.warn(
        "flash_attn not found. Flash-attention acceleration will be disabled. "
        "Install with: pip install flash-attn --no-build-isolation"
    )

# ── Transformers helper imports (API changed across versions) ─────────────────
try:
    ...
    apply_rotary_pos_emb = repeat_kv = rotate_half = None

_TRANSFORMERS_VERSION = version.parse(transformers.__version__)
_NEW_TRANSFORMERS = _TRANSFORMERS_VERSION >= version.parse("4.36.0")
...
def replace_llama_attn(use_flash_attn=True, use_full=False, inference=False):
    """
    Patch LlamaAttention with faster attention variants.

    Behaviour matrix:
      transformers >= 4.36  → native SDPA / flash_attention_2 is already built in;
                              manual patching is skipped and a recommendation is printed.
      transformers <  4.36 + flash_attn installed → flash-attn patching applied.
      transformers <  4.36 + flash_attn missing   → shift/group attention (no flash) applied.
    """
    if _NEW_TRANSFORMERS:
        # transformers ≥ 4.36 rewrote the LLaMA attention class; the old forward
        # signatures are incompatible.  These versions natively support Flash
        # Attention 2 via attn_implementation="flash_attention_2" and SDPA by
        # default, so manual patching is unnecessary.
        print(
            f"[llama_attn_replace] transformers {transformers.__version__} detected. "
            "Built-in SDPA / flash_attention_2 is used automatically — "
            "skipping manual attention patching."
        )
        return

    # ── Legacy transformers (< 4.36) patching path ───────────────────────────
    if use_flash_attn:
        if not FLASH_ATTN_AVAILABLE:
            warnings.warn(
                "[llama_attn_replace] flash_attn not installed; "
                "falling back to shift-group attention (forward_noflashattn)."
            )
        ...
        if inference:
            print("[llama_attn_replace] replacing attention for inference with flash attention")
            try:
                transformers.models.llama.modeling_llama.LlamaModel._prepare_decoder_attention_mask = (
                    _prepare_decoder_attention_mask_inference
                )
            except AttributeError:
                pass  # method removed in newer versions
