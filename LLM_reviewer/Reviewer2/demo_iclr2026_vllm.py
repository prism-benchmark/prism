...
DEFAULT_ENGINE_MAX_MODEL_LEN = 28672
...
def parse_args():
    parser = argparse.ArgumentParser(description="Reviewer2 ICLR2026 vLLM Batch Processing")
    parser.add_argument(
        "--grobid_dir",
        type=str,
        default="/datastore/npl/luannt/IHSD/Reviewer2/ICLR_2026/grobid_fulltext",
        help="Directory containing full-text files (.grobid.txt and/or .txt)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/datastore/npl/luannt/IHSD/Reviewer2/output_reviewer2_iclr2026",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Batch size for outer file iteration",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--paper_ids",
        type=str,
        default="/datastore/npl/luannt/IHSD/Reviewer2/ICLR_2026/data_subset/paper_ids.txt",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to a .txt file where missing/empty review paper IDs will be written after the run.",
    )
    parser.add_argument(
...
def calculate_dynamic_paper_token_limit(sequence_len: int) -> int:
    return min(MAX_PAPER_TOKENS, sequence_len - PHASE2_MAX_TOKENS - PROMPT_OVERHEAD_TOKENS)
        ...
        while max_model_len >= MIN_ENGINE_MAX_MODEL_LEN:
            ...
            try:
                ...
                if is_memory_pressure:
                    print(
                        "vLLM failed to initialize due to insufficient free VRAM at startup. "
                ...
                if estimated_max_model_len is not None and estimated_max_model_len < max_model_len:
                    ...
                    print(
                        "vLLM initialized model weights but did not have enough KV cache for the requested "
...
def calculate_max_new_tokens(input_token_count: int, base_max_output: int, min_output_tokens: int, sequence_len: int) -> int:
    available = sequence_len - input_token_count - SYSTEM_PROMPT_RESERVE
    safe_max = min(base_max_output, max(min_output_tokens, available - GENERATION_SAFETY_MARGIN))
    return max(min_output_tokens, safe_max)


def should_fallback_to_transformers(error_text: str) -> bool:
    fallback_markers = [
...
def load_transformers_model(model_path: str, config, effective_seq_len: int):
    ...
    model.generation_config.max_length = effective_seq_len
    model.eval()
    return model


def generate_with_transformers_backend(
    model,
    tokenizer,
    prompt: str,
    max_new_tokens: int,
    ...
    max_input_tokens = max(1, model.generation_config.max_length - max_new_tokens - SYSTEM_PROMPT_RESERVE)
    tokenized = tokenizer(
        prompt,
        return_tensors="pt",
        truncation=True,
    ...
    if prompt_token_count >= max_input_tokens:
        print(
            f"  ! Transformers prompt truncated to {prompt_token_count} tokens "
            f"before generation (limit={max_input_tokens})"
        )
    inputs = tokenized.to(input_device)
    min_new_tokens = min(64, max_new_tokens)
...
def generate_text(
    ...
    return generate_with_transformers_backend(
        model,
        tokenizer,
        prompt,
        max_tokens,
...
def compute_safe_batch_max_tokens(prompts, tokenizer, requested_max_tokens: int, effective_seq_len: int) -> int:
    if not prompts:
        return 0

    prompt_lengths = [count_tokens(prompt, tokenizer) for prompt in prompts]
    available_by_prompt = [effective_seq_len - prompt_length - SYSTEM_PROMPT_RESERVE for prompt_length in prompt_lengths]
...
def retry_empty_generations(
    ...
    for index in empty_indices:
        safe_max_tokens = compute_safe_batch_max_tokens(
            [prompts[index]], tokenizer, requested_max_tokens, effective_seq_len
        )
...
def main():
    ...
    try:
        llm, active_gpu_memory_utilization, active_max_model_len = load_vllm_with_retry(
            args.model_path,
            args.gpu_memory_utilization,
            min(args.max_model_len, effective_seq_len),
            args.max_num_batched_tokens,
        )
        effective_seq_len = min(effective_seq_len, active_max_model_len)
        print(f"Using effective sequence length: {effective_seq_len} tokens")
        print(f"Using gpu_memory_utilization: {active_gpu_memory_utilization}")
        print(f"Using max_model_len: {active_max_model_len}")
        print(f"Using max_num_batched_tokens: {args.max_num_batched_tokens}")
    except Exception as exc:
        error_text = str(exc)
        if not should_fallback_to_transformers(error_text):
            raise
        print(f"vLLM startup failed with recoverable error: {error_text}")
        model = load_transformers_model(args.model_path, config, effective_seq_len)
        use_vllm_backend = False
        print(f"Using effective sequence length: {effective_seq_len} tokens")
        print("Using backend: Transformers fallback")
    ...
    dynamic_paper_token_limit = calculate_dynamic_paper_token_limit(effective_seq_len)
    if dynamic_paper_token_limit <= 0:
        raise RuntimeError(
            f"Active sequence length {effective_seq_len} is too small for phase-2 generation budget."
        )
    print(f"Using max paper tokens: {dynamic_paper_token_limit}")
    ...
    for batch_index, batch_files in enumerate(iter_batches(pending_files, args.batch_size), start=1):
        ...
        for record in batch_records:
            try:
                phase1_prompt = build_phase1_prompt(record["paper_content"], tokenizer)
                phase1_prompt_tokens = count_tokens(phase1_prompt, tokenizer)
                safe_max_output_phase1 = calculate_max_new_tokens(
                    phase1_prompt_tokens,
                    PHASE1_MAX_TOKENS,
                    PHASE1_MIN_TOKENS,
                    effective_seq_len,
                )
                safe_max_output_phase1 = min(
                    safe_max_output_phase1,
                    compute_safe_batch_max_tokens(
                        [phase1_prompt], tokenizer, PHASE1_MAX_TOKENS, effective_seq_len
                    ),
                )
                if safe_max_output_phase1 < PHASE1_MIN_TOKENS:
                    failed.append((Path(record["file_path"]).name, "phase1 prompt too long: insufficient generation budget"))
                    print(f"  ✗ {record['paper_id']}: phase1 generation budget too small ({safe_max_output_phase1} tokens)")
                    continue

                print(f"  {record['paper_id']} phase1 max_tokens={safe_max_output_phase1}")
                phase1_output = generate_text(
                    phase1_prompt,
                    safe_max_output_phase1,
                    tokenizer,
                    use_vllm_backend,
                ...
                phase1_output = normalize_generated_text(phase1_output)
                phase1_output = retry_empty_generations(
                    llm,
                    [phase1_prompt],
                    [phase1_output],
                    tokenizer,
                ...
                if is_effectively_empty(phase1_output):
                    phase1_output = fallback_phase1_questions()

                phase2_prompt = build_phase2_prompt(
                    record["paper_content"],
                    record["compliance_guidance"],
                    phase1_output,
                    tokenizer,
                )
                phase2_prompt_tokens = count_tokens(phase2_prompt, tokenizer)
                safe_max_output_phase2 = calculate_max_new_tokens(
                    phase2_prompt_tokens,
                    PHASE2_MAX_TOKENS,
                    PHASE2_MIN_TOKENS,
                    effective_seq_len,
                )
                safe_max_output_phase2 = min(
                    safe_max_output_phase2,
                    compute_safe_batch_max_tokens(
                        [phase2_prompt], tokenizer, PHASE2_MAX_TOKENS, effective_seq_len
                    ),
                )
                if safe_max_output_phase2 < PHASE2_MIN_TOKENS:
                    failed.append((Path(record["file_path"]).name, "phase2 prompt too long: insufficient generation budget"))
                    print(f"  ✗ {record['paper_id']}: phase2 generation budget too small ({safe_max_output_phase2} tokens)")
                    continue

                print(f"  {record['paper_id']} phase2 max_tokens={safe_max_output_phase2}")
                ...
                review = normalize_generated_text(review)
                review = retry_empty_generations(
                    llm,
                    [phase2_prompt],
                    [review],
                    tokenizer,
                ...
                failed.append((Path(record["file_path"]).name, f"generation error: {exc}"))

    print("\n" + "=" * 70)
    print("ICLR2026 BATCH PROCESSING COMPLETE")
    print("=" * 70)
