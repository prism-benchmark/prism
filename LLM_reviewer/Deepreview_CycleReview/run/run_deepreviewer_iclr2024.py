#!/usr/bin/env python3
"""
DeepReviewer Pipeline Runner — ICLR2024 Dataset
Iterates over paired paper + review files from separate folders

Usage:
    python run_deepreviewer_iclr2024.py                  # run full pipeline
    python run_deepreviewer_iclr2024.py --check          # check environment only
    python run_deepreviewer_iclr2024.py --limit 2        # only process first 2 papers (for testing)
"""
...
# ── Load config ───────────────────────────────────────────────
try:
    import deepreviewer_config_iclr2024 as deepreviewer_config
except ImportError:
    print("ERROR: deepreviewer_config_iclr2024.py not found. Must be in the same directory.")
    sys.exit(1)
...
def check_environment():
    ...
    if mmd_folder.exists() and json_folder.exists():
        ...
        log(f"✓ MMD_FOLDER : {len(mmd_files)} paper files")
