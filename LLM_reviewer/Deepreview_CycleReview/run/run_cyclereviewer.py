"""
CycleReviewer-ML-Llama-3.1-8B Pipeline
Uses the official ai_researcher package.

Usage:
    python run_cycle_reviewer.py --check     # environment check only
    python run_cycle_reviewer.py --limit 2   # test run
    python run_cycle_reviewer.py --all-papers
    python run_cycle_reviewer.py             # full run
"""
...
def _resolve_paper_file(folder: str, paper_id: str) -> str | None:
    """Resolve a single paper ID to its text file path."""
...
def check_environment():
    ...
    print(f"[OK] {len(get_paper_pairs(paper_ids_filter))} matched paper pairs")
    print("=========================")
    return ok
...
def run_pipeline(limit: int = None, all_papers: bool = False):
    ...
    if paper_ids_filter is not None:
        print(f"Loaded {len(paper_ids_filter)} paper IDs from {PAPER_IDS_FILE}")
