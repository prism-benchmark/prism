#!/usr/bin/env python3
"""
DeepReviewer Pipeline Runner — ICML2025 Dataset
Iterates over paired paper text + .json files from separate folders

Usage:
    python run_deepreviewer_icml2025.py                  # run full pipeline
    python run_deepreviewer_icml2025.py --check          # check environment only
    python run_deepreviewer_icml2025.py --limit 2        # only process first 2 papers (for testing)
"""
...
# ── Load config ───────────────────────────────────────────────
try:
    import deepreviewer_config_icml2025 as deepreviewer_config
except ImportError:
    print("ERROR: deepreviewer_config_icml2025.py not found. Must be in the same directory.")
    sys.exit(1)
...
def load_paper_ids_filter():
    """Load paper IDs from file if configured. Returns set of IDs to process, or None for all."""
...
def verify_best_mode(generated_review):
    raw = generated_review[0].get("raw_text", "")
    openscholar_used = (
        "❓ Questions" in raw or
        "boxed_questions" in raw or
        "## Question" in raw
    )
    log("── Best Mode verification ──────────────────")
    log(f"  Mode set in config  : {deepreviewer_config.REVIEW_MODE}")
    log(f"  OpenScholar markers : {'✓ found' if openscholar_used else '✗ NOT found — check openscholar_api.log'}")
    log(f"  Raw output length   : {len(raw)} chars "
        f"({'✓ looks right' if len(raw) > 5000 else '⚠ unusually short'})")
    log("────────────────────────────────────────────")
    if not openscholar_used:
        log(f"  TIP: check logs at {deepreviewer_config.OPENSCHOLAR_DIR}/../openscholar_api.log")
...
if __name__ == "__main__":
    ...
    try:
        # Auth + load DeepReviewer once — stays in memory for all papers
        ...
        # Load paper IDs filter if configured
