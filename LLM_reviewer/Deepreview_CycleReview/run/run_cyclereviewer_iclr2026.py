"""
CycleReviewer-ML-Llama-3.1-8B Pipeline for ICLR2026
Uses the official ai_researcher package.

Usage:
    python run_cyclereviewer_iclr2026.py --check     # environment check only
    python run_cyclereviewer_iclr2026.py --limit 2   # test run
    python run_cyclereviewer_iclr2026.py --all-papers # process all papers
    python run_cyclereviewer_iclr2026.py             # full run (200 papers from subset)
"""
...
def truncate_paper_to_tokens(text: str, tokenizer, max_tokens: int = 16000) -> str:
    ...
    for marker in [". ", ".\n", "!\n", "?\n"]:
        ...
        if idx > max_tokens * 0.8:  # At least 80% of the way through
...
def load_paper(mmd_path: str) -> str:
    ...
    # Trim excessive whitespace and limit to ~12000 chars (~3000 tokens safety margin)
    text = text.rstrip()
    if len(text) > 250000:  # ~62k tokens, allow but warn
...
def _discover_paper_files(folder: str) -> dict:
    ...
    """
    result = {}
    # Try .grobid.txt first (double extension — must be stripped specially)
...
def get_paper_pairs(paper_ids_filter=None) -> list:
    ...
    if paper_ids_filter is not None:
        pairs = [pair for pair in pairs if pair[0] in paper_ids_filter]

    return pairs


def is_completed(paper_id: str) -> bool:
    return os.path.exists(os.path.join(OUTPUT_FOLDER, f"{paper_id}_result.json"))
...
def check_environment():
    print("=== ICLR2026 Environment Check ===")
    ok = True
...
def run_pipeline(limit: int = None, all_papers: bool = False):
    ...
    tokenizer = AutoTokenizer.from_pretrained(os.getenv("CYCLEREVIEWER_MODEL_PATH", os.path.join(HF_HOME, "CycleReviewer-8B")))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CycleReviewer pipeline for ICLR2026 dataset"
    )
