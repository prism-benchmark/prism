"""
Flaw-Identification Pipeline – ICML 2025 entry point.

Data paths  : E:\\Final_LLM_Reviewer_Data\\ICML2025\\
Default LLM : Google Gemini 2.5 Flash Lite  (google-genai library)

Key differences from ICLR pipelines:
  - Human review JSON uses ICML-structured rubric (no Weaknesses/Strengths keys).
    Fields used: Summary, Claims And Evidence, Methods And Evaluation Criteria,
    Theoretical Claims, Experimental Designs Or Analyses, Relation To Broader
    Scientific Literature, Essential References Not Discussed,
    Other Strengths And Weaknesses, Other Comments Or Suggestions,
    Questions For Authors.
  - Paper files are .grobid.txt (GROBID-extracted text)
  - SEA reviews have the same **Section:** format as ICLR 2025/2026
    → only Summary / Weaknesses / Questions are kept

Three modes
-----------
  cfi_only  – Step 1 (flaw extraction) + Step 2 (validation) + CFI metrics
              → outputs to output_cfi_icml2025/all_papers_results.jsonl
  all       – cfi_only + CPS coverage metrics
  cps_only  – CPS from a saved CFI JSONL (no paper text needed)
              → default cache: output_cfi_icml2025/all_papers_results.jsonl

Example commands
----------------
  python main_cfi_icml2025.py --mode cfi_only
  python main_cfi_icml2025.py --mode all
  python main_cfi_icml2025.py --mode cps_only
  python main_cfi_icml2025.py --mode cfi_only --model gemini-2.5-flash
"""
...
# Path configuration
# ---------------------------------------------------------------------------
from paths_config import ICML2025_DATA
HUMAN_FOLDER      = os.path.join(ICML2025_DATA, "human_reviews")
SEA_FOLDER        = os.path.join(ICML2025_DATA, "sea_icml2025")
TREE_FOLDER       = os.path.join(ICML2025_DATA, "tree_icml2025_2")
REVIEWER2_FOLDER  = os.path.join(ICML2025_DATA, "reviewer2_icml2025")
DEEPREVIEW_FOLDER = os.path.join(ICML2025_DATA, "deepreview_icml2025")
CYCLEREVIEW_FOLDER= os.path.join(ICML2025_DATA, "cyclereview_icml2025")
PAPERS_FOLDER     = os.path.join(ICML2025_DATA, "papers")
PAPER_IDS_FILE    = os.path.join(ICML2025_DATA, "paper_ids_200_icml2025.txt")
OUTPUT_DIR        = os.path.join(_HERE, "output_cfi_icml2025_2")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run flaw-identification pipeline on ICML 2025 data.",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to a cached CFI JSONL file (cps_only mode only).\n"
    ...
    parser.add_argument(
        ...
        help=(
            "LLM reviewer source:\n"
            "  sea         – SEA reviews from sea_icml2025/ (default)\n"
            "  tree        – Tree reviews from tree_icml2025/\n"
            "  reviewer2   – Reviewer2 reviews from reviewer2_icml2025/\n"
            "  deepreview  – DeepReview JSON from deepreview_icml2025/\n"
...
def process_single_paper(
    ...
    for idx, review_obj in enumerate(human_list):
        text = format_human_review_text_icml(review_obj)
        if text.strip():
...
def process_cps_from_cache(
    ...
    print(">> Computing CPS (from cached Step 1+2)...")
    cps_report = _compute_cps(
        pipeline, step1_flaws, step2_evals, human_reviews_dict, llm_review_text
    )
...
def _compute_cps(
    ...
    for r in rows:
        r["CPS_norm"] = (
            round((r["Raw_CPS"] - mn) / rng, 4) if rng > 0
            else (1.0 if r["Raw_CPS"] > 0 else 0.0)
        )
